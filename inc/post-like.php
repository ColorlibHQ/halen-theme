...
add_action( 'wp_enqueue_scripts', 'halen_sl_enqueue_scripts' );
function halen_sl_enqueue_scripts() {

	wp_enqueue_script( 'halen-post-likes', HALEN_DIR_JS_URI . 'post-likes.js', array( 'jquery' ), '0.5', false );

	wp_localize_script( 'halen-post-likes', 'simpleLikes', array(
		'ajaxurl' => admin_url( 'admin-ajax.php' ),
		'unlike'  => esc_html__( 'Unlike', 'halen' )
	) );
}
 ...
 * @since    0.5
 */
add_action( 'wp_ajax_nopriv_halen_process_simple_like', 'halen_process_simple_like' );
add_action( 'wp_ajax_halen_process_simple_like', 'halen_process_simple_like' );
function halen_process_simple_like() {
	// Fitness
	$nonce = isset( $_REQUEST['nonce'] ) ? sanitize_text_field( $_REQUEST['nonce'] ) : 0;
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( esc_html__( 'Not permitted', 'halen' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = esc_html__( 'Unlike', 'halen' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = esc_html__( 'Like', 'halen' );
		$icon = $icon_empty;
	}
...
function get_like_count( $like_count ) {
	$like_text = esc_html__( '', 'halen' );
	if ( is_numeric( $like_count ) && $like_count > 1 ) { 
		$number = sl_format_count( $like_count ) . esc_html__( ' Likes', 'halen' );
	} else {
		$number = sl_format_count( $like_count ) . esc_html__( ' Like', 'halen' );
	}
...
// User Profile List
add_action( 'show_user_profile', 'halen_show_user_likes' );
add_action( 'edit_user_profile', 'halen_show_user_likes' );
function halen_show_user_likes( $user ) { ?>
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'halen' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php esc_html__( 'You do not like anything yet.', 'halen' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
