...
final class Halen {
	...
	private function setup() {
		...
		$enqueu          = new halen_Enqueue();
		$enqueu->scripts = $this->enqueue();
		$enqueu->halen_scripts_enqueue_init();
	...
	public function support() {
		// content width
		$GLOBALS['content_width'] = apply_filters( 'halen_content_width', 751 );


		// text domain for translation.
		load_theme_textdomain( 'halen', HALEN_DIR_PATH . '/languages' );
		...
		// Site logo size
		add_image_size( 'halen_site_logo_94x33', 94, 33, true );

		// Hero right thumb size
		add_image_size( 'halen_hero_right_thumb_464x680', 464, 680, true );

		// Service section thumb size
		add_image_size( 'halen_service_man_thumb_372x655', 372, 655, true );
		add_image_size( 'halen_service_right_thumb_561x707', 561, 707, true );

		// Feature section thumb size
		add_image_size( 'halen_feature_right_illustration_thumb_588x341', 588, 341, true );
		add_image_size( 'halen_feature_left_phone_thumb_336x680', 336, 680, true );

		// Latest post thumbnail size
		add_image_size( 'halen_widget_post_thumb', 80, 80, true );

		// Single blog post image size
		add_image_size( 'halen_single_blog_750x375', 750, 375, true );
		add_image_size( 'halen_np_thumb', 60, 60, true );
		...
		register_nav_menus( array(
			'primary-menu' => esc_html__( 'Primary Menu', 'halen' ),
			'services'     => esc_html__( 'Services', 'halen' ),
			'useful-links' => esc_html__( 'Useful Links', 'halen' ),
		) );
	...
	private function enqueue() {
		...
		$cssPath = HALEN_DIR_CSS_URI;
		$jsPath  = HALEN_DIR_JS_URI;
	...
	private function google_font() {
		...
		if ( 'off' !== _x( 'on', 'Google font: on or off', 'halen' ) ) {
	...
	private function customizer_init(){
		...
		// Instantiate halen theme customizer
		$halen_theme_customizer = new halen_theme_customizer();
	}
	...
	public function set_elementor_flag(){
		update_option( 'halen_had_elementor', 'no' );
	}
	// Elementor dsiable default style
	public function elementor_desiable_default_style(){

		$nonce = $_POST['nonce'];
		if ( ! wp_verify_nonce( $nonce, 'halen-elementor-notice-nonce' ) ) {
			return;
		}
		$reply = $_POST['reply'];
		if ( ! empty( $reply ) ) {
			...
			update_option( 'halen_had_elementor', 'yes' );
		}
		die();
	...
	public function enqueue_elementor_theme_default_style(){
		...
		if ( $disabled_color_schemes === 'yes' && $disabled_typography_schemes === 'yes' ) {
			wp_enqueue_style( 'halen-elementor-default-style',  HALEN_DIR_CSS_URI. 'elementor-default-element-style.css', array(), $this->halen_version );
		}
	}
	// Enqueue elementor notice scripts
	public function enqueue_elementor_notice_script(){

		$had_elementor = get_option( 'halen_had_elementor' );

		if( $had_elementor == 'no' && self::check_elementor_preview_page() ){
			wp_enqueue_script( 'halen-elementor-notice', HALEN_DIR_JS_URI.'halen-elementor-notice.js', array('jquery'), '1.0', true );
			wp_localize_script(
				'halen-elementor-notice',
				'halenElementorNotice',
				array(
					'ajaxurl' => admin_url( 'admin-ajax.php' ),
					'nonce'   => wp_create_nonce( 'halen-elementor-notice-nonce' ),
				)
			);
		}
