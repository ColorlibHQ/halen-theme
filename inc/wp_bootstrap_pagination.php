...
function halen_pagination( $args = array() ) {
    
    $defaults = array(
        'range'           => 4,
        'custom_query'    => FALSE,
        'previous_string' => esc_html__( '&laquo;', 'halen' ),
        'next_string'     => esc_html__( '&raquo;', 'halen' ),
        'before_output'   => '<div class="halen-pagination-area col-12"><nav><ul class="pagination">',
        'after_output'    => '</ul></nav></div>'
    );
    
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'halen_pagination_defaults', $defaults )
    );
