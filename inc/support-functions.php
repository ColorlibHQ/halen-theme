...
function halen_post_cats() {
	...
	if ( $cats ) {
		...
		$categories .= '<ul class="nav"><li><span><i class="fa fm fa-th-list"></i>' . esc_html( 'Catagory :', 'halen' ) . '</span></li>';
...
// Blog categories
function halen_featured_post_cat(){
...
// social media
if ( ! function_exists( 'halen_social' ) ) {
	function halen_social( $args = array() ) {
		...
		$url = halen_opt( 'halen_social_url' );
...
// header cart count
function halen_cart_count( $class = '' ) {

	?>
	<div class="header-wrapicon2 <?php echo esc_attr( $class ); ?>">
		<img src="<?php echo esc_url( HALEN_DIR_ASSETS_URI ); ?>img/icons/icon-header-02.png" class="header-icon1 js-show-header-dropdown" alt="<?php esc_attr_e( 'ICON', 'halen' ); ?>">
...
function halen_contact7_form_content( $template, $prop ) {
...
add_filter( 'wpcf7_default_template', 'halen_contact7_form_content', 10, 2 );